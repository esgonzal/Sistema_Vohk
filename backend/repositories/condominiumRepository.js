...
async function findByAdminUserId(adminUserId = null) {
    const result = await pool.query(`
        SELECT condominium_id, admin_user_id, name, address, city, resident_camera_access,
               max_recurrent_invitations, max_temporary_duration_hours, max_express_duration_hours, created_at
        FROM condominium
...
async function getInvitationSettings(condominiumId, adminUserId) {
    const result = await pool.query(`
        SELECT condominium_id, max_recurrent_invitations, max_temporary_duration_hours, max_express_duration_hours
        FROM condominium
        WHERE condominium_id = $1 AND ($2::uuid IS NULL OR admin_user_id = $2)
        `, [condominiumId, adminUserId]);
    return result.rows[0];
...
async function updateInvitationSettings(condominiumId, adminUserId, maxRecurrentInvitations, maxTemporaryDurationHours, maxExpressDurationHours) {
    const result = await pool.query(`
        UPDATE condominium
        SET max_recurrent_invitations = $3,
            max_temporary_duration_hours = $4,
            max_express_duration_hours = $5
        WHERE condominium_id = $1 AND ($2::uuid IS NULL OR admin_user_id = $2)
        RETURNING condominium_id, max_recurrent_invitations, max_temporary_duration_hours, max_express_duration_hours
        `, [condominiumId, adminUserId, maxRecurrentInvitations, maxTemporaryDurationHours, maxExpressDurationHours]);
    return result.rows[0];
}
...
async function findCondominiumTreeRows(adminUserId = null) {
    const result = await pool.query(`
        SELECT c.condominium_id, c.name AS condominium_name, c.address, c.city, c.resident_camera_access,
               ...
               b.building_id, b.name AS building_name, b.floor_count,
               z.zone_id, z.name AS zone_name, z.created_at AS zone_created_at
        FROM condominium c
...
async function findUnitTreeRows(condominiumId) {
    const result = await pool.query(`
        SELECT c.condominium_id, c.name AS condominium_name, c.address, c.city, c.resident_camera_access,
               b.building_id, b.name AS building_name, b.floor_count,
               u.unit_id, u.name, u.room_no, u.floor,
               au.user_id, au.legal_name, au.email, au.sip_identity, au.role, au.active, ru.is_primary
        FROM condominium c
        LEFT JOIN building b ON b.condominium_id = c.condominium_id
...
module.exports = {
    findById, findByIdAndAdmin, findByAdminUserId, createCondominium, updateCondominium, deleteCondominium,
    countBuildingsByCondominium, updateResidentCameraAccess, getInvitationSettings, updateInvitationSettings,
