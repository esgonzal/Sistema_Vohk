...
async function getSummary(adminUserId) {
    const result = await pool.query(
        `
        SELECT
            COUNT(DISTINCT c.condominium_id)::int AS condominiums,
            COUNT(DISTINCT b.building_id)::int AS buildings,
            COUNT(DISTINCT u.unit_id)::int AS units,
            COUNT(DISTINCT ru.user_id)::int AS residents,
            COUNT(DISTINCT d.device_id)::int AS devices,
            COUNT(DISTINCT i.intercom_id)::int AS intercoms,
            COUNT(
                DISTINCT CASE
                    WHEN d.type = 'camera'
                    THEN d.device_id
...
async function getCondominiums(adminUserId) {
    const result = await pool.query(
        ...
        LEFT JOIN device d ON d.zone_id = z.zone_id
        WHERE c.admin_user_id = $1
        GROUP BY c.condominium_id, c.name, c.address, c.city
        ORDER BY c.name
        `,
        [adminUserId]
    );
...
async function getDeviceSummary(adminUserId) {
    const result = await pool.query(
        `
        SELECT
            COUNT(DISTINCT d.device_id)::int AS total,
            COUNT(DISTINCT d.device_id) FILTER (WHERE d.last_seen_at >= NOW() - INTERVAL '5 minutes')::int AS online,
            COUNT(DISTINCT d.device_id) FILTER (WHERE d.last_seen_at < NOW() - INTERVAL '5 minutes' OR d.last_seen_at IS NULL)::int AS offline,
            COUNT(DISTINCT d.device_id) FILTER (WHERE d.type = 'camera')::int AS cameras,
            COUNT(DISTINCT d.device_id) FILTER (WHERE d.type = 'camera' AND d.last_seen_at >= NOW() - INTERVAL '5 minutes')::int AS cameras_online,
            COUNT(DISTINCT d.device_id) FILTER (WHERE d.type = 'intercom')::int AS intercoms,
            COUNT(DISTINCT d.device_id) FILTER (WHERE d.type = 'intercom' AND d.last_seen_at >= NOW() - INTERVAL '5 minutes')::int AS intercoms_online
        FROM device d
